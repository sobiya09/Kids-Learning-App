...
class ArithmeticGamePage extends StatefulWidget {
  ...
  const ArithmeticGamePage({Key? key, required this.operation})
      : super(key: key);

  @override
  State<ArithmeticGamePage> createState() => _ArithmeticGamePageState();
}
...
class _ArithmeticGamePageState extends State<ArithmeticGamePage> {
  ...
  Future<void> loadHighScore() async {
    ...
    setState(() {
      highScore = prefs.getInt('highScore_${widget.operation}') ?? 0;
    });
  }

  Future<void> saveHighScore(int newScore) async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setInt('highScore_${widget.operation}', newScore);
    setState(() {
      highScore = newScore;
    });
  }
